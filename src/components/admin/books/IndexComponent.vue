    ...
    <div class="container">
        <div class="card border-0 shadow rounded-3 my-3">
            <div class="card-body p-4 p-sm-5">
                <h5 class="card-title mb-2 fw-light fs-5">Filters</h5>
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4 mt-2">
                        <div class="mb-3">
                            <button class="btn btn-success btn-lg mt-1 ml-2" @click="applyFilter()">Apply</button>&nbsp;
                            <button class="btn btn-primary btn-lg mt-1" @click="clearFilter()">Clear</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4 p-sm-5">
                <div class="row">
                    <div class="col-md-12 mt-2">
                        <h5 class="card-title mb-2 fw-light fs-5">Books</h5>
                        <router-link class="btn btn-primary" :to="{ name: 'admin.books.create' }">Add New
                            Book</router-link>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12 mt-2">
                        <EasyDataTable :headers="headers" :items="items" v-model:server-options="serverOptions"
                            ...
                            <template #item-image="item">
                                <a :href="item.image" target="_blank"><img :src="item.image" width="50"
                                        height="50" /></a>
                            </template>
                            <template #item-action="item">
                                <router-link class="btn btn-primary btn-sm"
                                    :to="{ name: 'admin.books.edit', params: { id: item.id } }">Edit</router-link>
                                <button class="btn btn-danger btn-sm" @click="deleteBook(item.id)">Delete</button>
                            </template>
                        </EasyDataTable>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        async getBooks() {
            ...
            var response = await this.adminApi('POST', 'books/list', this.payload)
            if (response) {
